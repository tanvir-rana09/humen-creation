...
const TestimonialCarousel = () => {
  ...
  // 🚀 NEW: refs for exact form targeting
  ...
  const yellowSectionInView = useInView(yellowSectionRef, {
    once: true,
    threshold: 0.3,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus("ok");
      setFormData({ fullName: "", email: "", purpose: "Not sure yet" });
    } catch (err) {
  ...
  // 🚀 NEW: make #waitlist deep link go to the exact card with sticky-nav offset + focus
  useEffect(() => {
    const scrollWithOffset = (el) => {
      if (!el) return;
      // Try to read sticky nav height if present
      const nav = document.querySelector(
        'nav.sticky, .sticky.top-0, nav[role="navigation"]'
      );
      const offset = (nav?.offsetHeight ?? 72) + 12;
      const y = el.getBoundingClientRect().top + window.pageYOffset - offset;
      window.scrollTo({ top: Math.max(y, 0), behavior: "smooth" });
      // focus first field a bit later so browser is at final position
      setTimeout(() => firstInputRef.current?.focus?.(), 350);
    };

    const goIfHash = () => {
      if (window.location.hash === "#waitlist") {
        scrollWithOffset(waitlistCardRef.current);
      }
    };

    goIfHash();
    window.addEventListener("hashchange", goIfHash);
    return () => window.removeEventListener("hashchange", goIfHash);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white font-sans pt-28 ">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 py-8 pt-20 ">
        <div
          ref={testimonialRef}
          className="flex flex-col lg:flex-row justify-between items-start gap-12 mb-40"
        >
          {/* Left - Title */}
          <motion.div
            initial="hidden"
          ...
          >
            <h1 className="relative text-4xl sm:text-5xl lg:text-6xl font-light text-gray-900 leading-[1.1] tracking-tight">
              ...
              <img
                src={vector38}
                alt=""
                className="hidden lg:block absolute bottom-20 left-[6.9rem] w-8 lg:w-7 transform rotate-[80deg]"
              />
            </h1>
          </motion.div>
        ...
        <div ref={yellowSectionRef} className="relative mb-20">
          ...
          >
            <div className="grid lg:grid-cols-2">
              ...
              >
                {/* 👇 EXACT SCROLL TARGET: give this the ID and a ref */}
                <div
                  id="waitlist"
                  ref={waitlistCardRef}
                  className="bg-white border-2 border-gray-400 rounded-2xl xl:rounded-4xl p-4 xl:p-8 shadow-xl max-w-3xl w-full scroll-mt-28"
                >
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-fit bg-teal-950 text-white py-3 px-6 rounded-full hover:bg-teal-900 transition-colors font-medium disabled:opacity-70 disabled:cursor-not-allowed"
                    >
        ...
        {/* Footer (give it an id for fallbacks) */}
        <footer
          id="footer"
          ref={footerRef}
          className="flex flex-col lg:flex-row justify-between items-start lg:items-center pt-12 border-t border-gray-200"
        >
          <motion.div
            initial="hidden"
